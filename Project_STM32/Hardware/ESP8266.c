...
/**
 * @brief  调试辅助函数：显示串口接收缓存的前32个字符
 * @note   用于快速排查AT指令响应异常问题，仅调试用
 * @param  无
 * @retval 无
 */
void Debug_ShowRX(void)
{
    OLED_ShowString(2, 1, "RX:");                 // 显示RX标识
    OLED_ShowString(2, 4, (char *)USART1_RX_BUF); // 显示接收缓存内容
}
...
uint8_t ESP8266_SendJSON(const char *ip, uint16_t port, const char *json)
{
    ...
    while (retry < 2) // 重试机制，提升连接成功率
    {
        ...
        Debug_ShowRX(); // 显示TCP错误（如端口不可达/连接拒绝）
        Delay_ms(1500);
    ...
    ESP8266_SendCmd("AT+CIPCLOSE", "OK", 1000);
    OLED_Clear();
    OLED_ShowString(1, 1, "All Success!");
    Delay_ms(2000);
    return 1;
}
